...
contract GatewayEVMTest is Test, IGatewayEVMErrors, IGatewayEVMEvents, IReceiverEVMEvents, IERC20CustodyEvents {
    ...
    function testUpgradePreservesDepositPauseFunctions() public {
        vm.startPrank(owner);
        gateway.setDepositPaused(true);
        ...
        assertTrue(gatewayUpgradeTest.depositPaused());
        assertTrue(gatewayUpgradeTest.depositAllowedAssets(address(0)));
        assertTrue(gatewayUpgradeTest.depositAllowedAssets(address(zeta)));

        vm.startPrank(owner);
        gatewayUpgradeTest.setDepositPaused(false);
        assertFalse(gatewayUpgradeTest.depositPaused());
        vm.stopPrank();
    }
}
...
contract GatewayEVMInboundTest is Test, IGatewayEVMErrors, IGatewayEVMEvents, IReceiverEVMEvents, INotSupportedMethods {
    ...
    function testDepositPauseAllowsZetaByDefault() public {
        ...
        assertTrue(gateway.depositAllowedAssets(address(zeta)));
    ...
    function testCallNotAffectedByDepositPause() public {
        bytes memory payload = abi.encodeWithSignature("hello(address)", destination);
