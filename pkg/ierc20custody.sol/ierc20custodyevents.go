...
// IERC20CustodyEventsStrandedFundsRefundedIterator is returned from FilterStrandedFundsRefunded and is used to iterate over the raw logs and unpacked data for StrandedFundsRefunded events raised by the IERC20CustodyEvents contract.
type IERC20CustodyEventsStrandedFundsRefundedIterator struct {
	Event *IERC20CustodyEventsStrandedFundsRefunded // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *IERC20CustodyEventsStrandedFundsRefundedIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(IERC20CustodyEventsStrandedFundsRefunded)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *IERC20CustodyEventsStrandedFundsRefundedIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *IERC20CustodyEventsStrandedFundsRefundedIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// IERC20CustodyEventsStrandedFundsRefunded represents a StrandedFundsRefunded event raised by the IERC20CustodyEvents contract.
type IERC20CustodyEventsStrandedFundsRefunded struct {
	To     common.Address
...
func (_IERC20CustodyEvents *IERC20CustodyEventsFilterer) FilterStrandedFundsRefunded(opts *bind.FilterOpts, to []common.Address, token []common.Address) (*IERC20CustodyEventsStrandedFundsRefundedIterator, error) {
	...
	logs, sub, err := _IERC20CustodyEvents.contract.FilterLogs(opts, "StrandedFundsRefunded", toRule, tokenRule)
	if err != nil {
		return nil, err
	}
	return &IERC20CustodyEventsStrandedFundsRefundedIterator{contract: _IERC20CustodyEvents.contract, event: "StrandedFundsRefunded", logs: logs, sub: sub}, nil
}
...
//
// Solidity: event StrandedFundsRefunded(address indexed to, address indexed token, uint256 amount)
func (_IERC20CustodyEvents *IERC20CustodyEventsFilterer) WatchStrandedFundsRefunded(opts *bind.WatchOpts, sink chan<- *IERC20CustodyEventsStrandedFundsRefunded, to []common.Address, token []common.Address) (event.Subscription, error) {
	...
	logs, sub, err := _IERC20CustodyEvents.contract.WatchLogs(opts, "StrandedFundsRefunded", toRule, tokenRule)
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
		...
		for {
			...
			case log := <-logs:
				// New log arrived, parse the event and forward to the user
				event := new(IERC20CustodyEventsStrandedFundsRefunded)
				if err := _IERC20CustodyEvents.contract.UnpackLog(event, "StrandedFundsRefunded", log); err != nil {
					return err
				}
				event.Raw = log
...
//
// Solidity: event StrandedFundsRefunded(address indexed to, address indexed token, uint256 amount)
func (_IERC20CustodyEvents *IERC20CustodyEventsFilterer) ParseStrandedFundsRefunded(log types.Log) (*IERC20CustodyEventsStrandedFundsRefunded, error) {
	event := new(IERC20CustodyEventsStrandedFundsRefunded)
	if err := _IERC20CustodyEvents.contract.UnpackLog(event, "StrandedFundsRefunded", log); err != nil {
		return nil, err
	}
