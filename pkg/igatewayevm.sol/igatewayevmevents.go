...
// IGatewayEVMEventsUpdatedAdditionalActionFeeIterator is returned from FilterUpdatedAdditionalActionFee and is used to iterate over the raw logs and unpacked data for UpdatedAdditionalActionFee events raised by the IGatewayEVMEvents contract.
type IGatewayEVMEventsUpdatedAdditionalActionFeeIterator struct {
	Event *IGatewayEVMEventsUpdatedAdditionalActionFee // Event containing the contract specifics and raw log
...
// are any more events found. In case of a retrieval or parsing error, false is
// returned and Error() can be queried for the exact failure.
func (it *IGatewayEVMEventsUpdatedAdditionalActionFeeIterator) Next() bool {
	// If the iterator failed, stop iterating
	if it.fail != nil {
		return false
	}
	...
	if it.done {
		select {
		case log := <-it.logs:
			it.Event = new(IGatewayEVMEventsUpdatedAdditionalActionFee)
			if err := it.contract.UnpackLog(it.Event, it.event, log); err != nil {
				it.fail = err
				return false
			}
...
// Error returns any retrieval or parsing error occurred during filtering.
func (it *IGatewayEVMEventsUpdatedAdditionalActionFeeIterator) Error() error {
	return it.fail
}

// Close terminates the iteration process, releasing any pending underlying
// resources.
func (it *IGatewayEVMEventsUpdatedAdditionalActionFeeIterator) Close() error {
	it.sub.Unsubscribe()
	return nil
}

// IGatewayEVMEventsUpdatedAdditionalActionFee represents a UpdatedAdditionalActionFee event raised by the IGatewayEVMEvents contract.
type IGatewayEVMEventsUpdatedAdditionalActionFee struct {
	OldFeeWei *big.Int
	NewFeeWei *big.Int
	Raw       types.Log // Blockchain specific contextual infos
}
...
func (_IGatewayEVMEvents *IGatewayEVMEventsFilterer) FilterUpdatedAdditionalActionFee(opts *bind.FilterOpts) (*IGatewayEVMEventsUpdatedAdditionalActionFeeIterator, error) {

	logs, sub, err := _IGatewayEVMEvents.contract.FilterLogs(opts, "UpdatedAdditionalActionFee")
	if err != nil {
		return nil, err
	}
	return &IGatewayEVMEventsUpdatedAdditionalActionFeeIterator{contract: _IGatewayEVMEvents.contract, event: "UpdatedAdditionalActionFee", logs: logs, sub: sub}, nil
}
...
//
// Solidity: event UpdatedAdditionalActionFee(uint256 oldFeeWei, uint256 newFeeWei)
func (_IGatewayEVMEvents *IGatewayEVMEventsFilterer) WatchUpdatedAdditionalActionFee(opts *bind.WatchOpts, sink chan<- *IGatewayEVMEventsUpdatedAdditionalActionFee) (event.Subscription, error) {

	logs, sub, err := _IGatewayEVMEvents.contract.WatchLogs(opts, "UpdatedAdditionalActionFee")
	if err != nil {
		return nil, err
	}
	return event.NewSubscription(func(quit <-chan struct{}) error {
